...
if ($this->mode == 'update') {
    ...
    if ($ok) {
        ...
        $commands = array('command', 'status', 'level', 'mode','disco');
        if ($rec['DEVICE_TYPE'] != '0') {
            $commands[] = 'color';
            $commands[] = 'white';
        }
        $total = count($commands);
        for ($i = 0; $i < $total; $i++) {
            $cmd_rec = SQLSelectOne("SELECT ID FROM milight_commands WHERE DEVICE_ID=" . $rec['ID'] . " AND TITLE LIKE '" . $commands[$i] . "'");
            if (!$cmd_rec['ID']) {
                $cmd_rec = array();
